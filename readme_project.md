...
- >`python` [hastagExperiment.py](src/experiments/hastagExperiment.py) `dataset=<use-full-or-partial-dataset> load_path=<trained-model-path> freq=500 prob=0.7` This script extracts hashtags and finds how much a hashtag matters. It is used to produce the hashtag analysis in the project's final report.
...
│   ├── [experimentConfigs](src/experiments/) Here are the experiment configurations. There are many json files in here each describing a series of experiments. This helps identify which are the best combinations of parameters/models/preprocessors to solve the problem.
