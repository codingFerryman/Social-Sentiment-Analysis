...
def _hashtag_matters(data_line: pd.Series, freq_threshold: float, prob_threshold: float):
    row = data_line
    _golden = row['golden']
    _text = row['text']
        ...
        if _w.startswith('#') and len(_w) > 1:
            if _w[1:] in hashtag_dict.keys():
                ...
                if (pos_freq > freq_threshold or neg_freq > freq_threshold) and (
                    ...
                    _neg_prob += neg_ratio * 2
                    _pos_prob += pos_ratio * 2
    _pred_prob = torch.softmax(torch.tensor([_neg_prob, _pos_prob]), dim=-1)
